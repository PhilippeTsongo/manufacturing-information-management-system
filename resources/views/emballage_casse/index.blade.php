                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuSplitButton1">
                                        @foreach($mois as $month)
                                            <a target="blank" href="{{ route('rapport.casse.emballage', ['month' => $month->name ]) }}" method="GET" class="dropdown-item">{{ $month->name }}</a>
                                        @endforeach                             
                                    </div>
                                </div>
                            ...
                            <div class="card">
                              <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    ...
                                    <tbody>
                                        @foreach($emballage_casses as $emballage_casse)
                                        <tr>
                                            <td>{{ $emballage_casse->number }}</td>
                                            <td>
                                                @if($emballage_casse->emballage)
                                                    {{ $emballage_casse->emballage->name . ' '. $emballage_casse->emballage->emballage_number }}
                                                @endif
                                            </td>
                                            <td>
                                                @if($emballage_casse->emballage)
                                                    @if($emballage_casse->emballage->type_emballage)
                                                        {{ $emballage_casse->emballage->type_emballage->name}}</td>
                                                    @endif
                                                @endif
                                            </td>
                                            <td>
                                                @if($emballage_casse->emballage)
                                                    @if($emballage_casse->emballage->unit)    
                                                        {{ $emballage_casse->quantity }} {{ $emballage_casse->emballage->unit->name }}
                                                    @endif
                                                @endif
                                            </td>
                                            <td>{{ $emballage_casse->date_emballage_casse }}</td>
                                            <td>
                                                @if(Auth()->check())
                                                    @if(Auth::user()->userType->name == 'Administrateur' OR Auth::user()->userType->name == 'Producteur')
                                                    <div class="row">
                                        ...
                                        {{ $emballage_casses->links('vendor.pagination.bootstrap-5')}}
                                    </tbody>
                                  </table>
                                </div>
                              </div>
